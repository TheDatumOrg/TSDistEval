function RunClusteringHierComplete(DataSetStartIndex, DataSetEndIndex, DistanceIndex)  
                    ...
                    for w=1:length(Params)
                        for wprime = 1:length(Params2)
                            [DMTRAIN,~] = DistanceToDM(DistanceIndex,Datasets,i,Methods, Params(w), Params2(wprime));
                             ...
                             Results(i,2) = Params2(wprime);
                             Results(i,3) = RandIndex(labels, DS.TrainClassLabels);
                             
                             dlmwrite( strcat('./RunClusteringHierComplete/', 'RESULTS_RunClusteringHierComplete_', char(Methods(DistanceIndex)),'_', num2str(Params(w)),'_', num2str(Params2(wprime)), '_',  num2str(i)), Results, 'delimiter', '\t');
